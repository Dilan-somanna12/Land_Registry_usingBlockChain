...
class table extends Component {
  ...
  render() {
    ...
    console.log('Buyer_Table - assetList from props:', assetList)
    return (
          ...
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {assetList && assetList.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={columns.length} align="center" style={{ padding: '40px' }}>
                    <div style={{ color: '#6b7280', fontSize: '16px' }}>
                      No available properties found. Properties will appear here once owners make them available.
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                ...
                return (
                  <TableRow hover role="checkbox" key={row.property}>
                    {columns.map((column) => {
                      const value = row[column.id]
                      return (
                        <TableCell key={column.id} align={column.align}>
                          {column.id == 'property' ? (
                            <span style={{ fontFamily: 'monospace', fontWeight: 'bold', color: '#4f46e5' }}>
                              #{String(value || 'N/A')}
                            </span>
                          ) : column.id == 'isAvailable' ? (
                            (() => {
                              // Debug log
                              if (row['property'] == 4416149325197) {
                                console.log('Property #4416149325197 - isAvailable value:', value, 'Type:', typeof value)
                              }
                              ...
                              } else if (status === 'GovtApproved') {
                                return <div style={{ color: '#ef4444' }}>Unavailable</div>
                            ...
                            (() => {
                              const govtStatus = String(value || '').trim()
                              if (govtStatus === 'Approved') {
                                return (
                                  <span style={{ 
                                    color: '#10b981', 
                                    fontWeight: 'bold',
                              ...
                              } else if (govtStatus === 'Not Approved') {
                                return (
                                  <span style={{ 
                                    color: '#ef4444', 
                                    fontWeight: 'bold',
